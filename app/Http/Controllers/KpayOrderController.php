...
class KpayOrderController extends Controller
{
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('transaction_image')) {
            ...
            $file->storeAs('topups', $filename, 'public');
            $data['transaction_image'] = $filename;
        }

        $zoneId = $request->input('zone_id');
        if (! $data['server_id'] && $zoneId) {
    ...
    public function adminIndex(Request $request)
    {
        ...
        return view('admin.kpay_orders', compact('orders', 'statuses', 'games', 'paymentMethods'));
    }
        ...
        try {
            ...
            if ($order->game_type === 'mlbb') {
                ...
                if (! ($result['success'] ?? false)) {
                    ...
                    return redirect()->back()->with('error', 'PUBG order failed: '.$message);
                }
                ...
                if (! ($result['ok'] ?? false)) {
                    ...
                    return redirect()->back()->with('error', 'MCGG order failed: '.$message);
                }
    ...
    public function deleteOldOrders(Request $request)
    {
        ...
        $count = KpayOrder::whereDate('created_at', '<', $date)->delete();
    ...
    public function fetchOrders(Request $request)
    {
        ...
        return response()->json([
            'html' => view('admin.partials.kpay_orders_table', compact('orders'))->render(),
